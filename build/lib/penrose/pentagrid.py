...
class Pentagrid(BaseSprite):
    ...
    def __init__(self, size) -> None:
        super().__init__(size)
        # self.mathpg = MathPentagrid(PenroseMap(array([.0,.1,.2,.3,-.6], float)))
        self.mathpg = MathPentagrid(NotQuitePenroseMap(array([.0,.1,.2,.3,-.6], float)))
        self.texture = None
    ...
    def get_intersections(self, lattices:list[Lattice]):
        ...
        intersections = zeros((intersectioncount, 4))
        n = 0
